...
const envSchema = z.object({
  ...
  JWT_SECRET: z.preprocess((v) => (v === '' ? undefined : v), z.string().min(16).optional()),
  ...
  DB_MODE: z.enum(['turso', 'sqlserver', 'sqlite']).optional(),
  ...
  SEED_SQL_ON_START: z.string().optional(),
...
export const serverConfig = {
  ...
  db: {
    mode: (parsed.DB_MODE ?? '') as 'turso' | 'sqlserver' | 'sqlite' | '',
    server: parsed.AZURE_SQL_SERVER || parsed.DB_SERVER || '',
    port: parsed.DB_PORT || 1433,
    database: parsed.AZURE_SQL_DATABASE || parsed.DB_DATABASE || '',
    user: parsed.AZURE_SQL_USER || parsed.DB_USER || '',
    password: parsed.AZURE_SQL_PASSWORD || parsed.DB_PASSWORD || '',
    sqlitePath: parsed.SQLITE_DB_PATH || parsed.DB_SQLITE_PATH || '',
    tursoUrl: parsed.TURSO_DATABASE_URL || parsed.TURSO_DB_URL || parsed.DATABASE_URL || '',
    tursoToken: parsed.TURSO_AUTH_TOKEN || parsed.TURSO_TOKEN || parsed.DATABASE_AUTH_TOKEN || '',
    seedSqlOnStart: parsed.SEED_SQL_ON_START === 'true',
  },
  localAdmin: {
